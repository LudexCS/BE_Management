import {
    adminSearchGameByChoseongService, adminSearchGameByEmbeddingSimilarityService, adminSearchGameByKeywordLikeService,
...
export const adminSearchGameControl = async (keyword: string) => {
    if (!keyword?.trim()) {
        throw new Error("Keyword must be provided");
    }
    if (canBeChoseong(keyword[0])) {
        return await adminSearchGameByChoseongService(keyword);
    }
    else {
        const byKeyword = await adminSearchGameByKeywordLikeService(keyword);
        const byEmbedding = await adminSearchGameByEmbeddingSimilarityService(keyword);
